# Security Policy

## Supported versions

Security fixes are made against the latest published Calm release.

## Reporting a vulnerability

Please do not open a public issue for a suspected vulnerability. Use
[GitHub's private vulnerability reporting form](https://github.com/raisulchowdhury/calm-nnwtheme/security/advisories/new)
and include:

- the affected Calm and NetNewsWire versions
- the device and OS version
- a minimal reproduction or affected feed/article
- the impact you observed

Calm is a static NetNewsWire theme, but its template runs inside a reader view.
Reports involving unexpected script execution, content escaping the article
surface, unsafe external navigation, or package tampering are especially
useful.
